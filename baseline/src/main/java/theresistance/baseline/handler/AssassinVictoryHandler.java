...
/**
 * A post round victory handler that checks to see if one of the sides have won
 * on rounds or not. If the evil side has won on rounds, then evil wins. If the
 * good side does, then the
 * {@link AssassinVictoryHandlerTest#WAITING_FOR_ASSASSINATION} property is set
 * to true in the game's extra info.
 */
public class AssassinVictoryHandler extends RoundVictoryHandler
{
	/**
	 * Game extra info property indicating that an the assassin should try to
	 * assassinate Merlin.
	 */
